...
- **Machine high row:** uses the chest-supported-row model—45% lats, 50% mid/lower traps and rhomboids, and 5% upper traps—with biceps credited as a secondary muscle. The fixed torso support removes the unsupported-row erector allocation while the high pulling path retains both lat and scapular-retractor credit.
